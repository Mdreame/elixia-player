    ...
    useEffect(() => {
      ...
      async function fetchUrl() {
        ...
        try {
          ...
          if (mounted) {
            setError("获取播放链接失败，请配置 Cookie 后重试");
            console.error(err);
          }
        } finally {
          if (mounted) {
    ...
    return (
      <div className="relative overflow-hidden rounded-2xl shadow-xl bg-zinc-900 border border-white/10 group h-72 md:h-80">
        ...
        <div className="relative p-4 md:p-6 flex flex-col justify-between h-full text-white z-10">
          ...
          {error && <p className="absolute bottom-24 left-0 right-0 text-xs text-red-300/70 text-center py-1 animate-in fade-in duration-300">{error}</p>}
        </div>
      </div>
    );
  }
